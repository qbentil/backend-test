...
export const ChooseDoctor = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const patient = await Services.optDoctor(req.tokenPayload?.id!, req.body.doctor);
...
export const patientProfile = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const patientProfileData = await Services.PatientProfile(req.tokenPayload?.id!);
...
export const MyReminders = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const reminders = await Services.getReminder({patient: req.tokenPayload?.id!});
